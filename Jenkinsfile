...
pipeline {
  ...
  "commands": ["git clone https://github.com/sanjayambatkar/risklab-ui.git"]
}' > parameters.json
                        bat script: 'aws ssm send-command --document-name "AWS-RunShellScript" --document-version "1" --targets "[{\"Key\":\"InstanceIds\",\"Values\":[\"i-002953cac36e74bd0\"]}]" --parameters "file://parameters.json" --timeout-seconds 600 --max-concurrency "50" --max-errors "0" --region ap-south-1', returnStatus: true
                    }
                }
            }
        }
